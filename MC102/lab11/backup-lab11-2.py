...
class Masmorra:
    def __init__(
        ...
        for entidade in [self._heroi] + self._monstros:
            entidade.limites = dimensoes

    def passo(self) -> bool:
        for objeto in (
            [self._heroi] + [self._saida] + self._items + self._monstros
        ):
            objeto.passo()
            if (
                objeto.posicao == self._heroi.posicao
                and id(objeto) != id(self._heroi)
                and not self._heroi.esta_morto
    ...
    def __str__(self) -> str:
        ...
        for objeto in (
            self._items + self._monstros + [self._saida] + [self._heroi]
        ):
