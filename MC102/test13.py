...
actual_out_dir = join(lab_dir, "images")
if not exists(actual_out_dir):
    os.mkdir(actual_out_dir)

solution_dir = join(lab_dir, "solution")
solutions = [
    join(solution_dir, f)
    for f in os.listdir(solution_dir)
    if isfile(join(solution_dir, f))
]
solutions.sort()
...
for in_file, solution_file in zip(in_files, solutions):
    test_name = splitext(basename(solution_file))[0]
    out_actual_file = join(actual_out_dir, test_name + ".pgm")
    out_expected_file = join(solution_dir, test_name + ".pgm")
    os.system(f"python3 {program_file} < {in_file}")
        ...
        with open(out_actual_file, "r") as actual:
            with open(out_expected_file, "r") as expected:
                ...
                if str_actual == str_expected:
                    print(
                        f"{COLOR_OKGREEN}Teste {test_name}: Sucesso{COLOR_END}",
                    )
                else:
                    print(
                        f"{COLOR_FAIL}Teste {test_name}: Falha{COLOR_END}",
                    )
                    # print(diff_strings(str_expected, str_actual))
                    # O seguinte codigo omite os erros caso ja tenha mostrado uma falha
                    if view_fail:
                        print(diff_strings(str_expected, str_actual))
                        view_fail = False
                    ...
                    #     list(
                    #         differ.compare(
                    #             str_expected.splitlines(keepends=True),
                    #             str_actual.splitlines(keepends=True),
                    #         )
                    #     )
                    # )
